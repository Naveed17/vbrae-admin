...
const PagesRow = ({ row, columns, key = 0, handleTableAction }) => {
    ...
    <TableRow hover tabIndex={-1} key={key}>
      {columns.map((column) => {
        ...
        if (column.id === 'visibility') {
          value = (
            <Chip
              label={row.visibility === 'visible' ? 'Visible' : 'Hidden'}
              size="small"
              color={row.visibility === 'visible' ? 'success' : 'default'}
              variant="outlined"
            />
          );
        } else if (column.id === 'page_type') {
          value = (
            <Chip
              label={row.page_type}
              size="small"
