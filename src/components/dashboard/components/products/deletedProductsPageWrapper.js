...
const rows = [
    {
        ...
        checkbox: false,
        product: {
            title: 'Divine Knockout Epic Games CD Key',
            image: 'https://cdn.vbrae.com/images/uploads/images/202507/84008c_97763.webp',
            url: 'https://vbrae.com/divine-knockout-epic-games-cd-key'
        },
        sku: '',
        product_type: 'Digital',
        category: 'WEEKLY DEALS',
        user: { name: 'Freaky', url: 'https://vbrae.com/profile/freaky' },
        date: '2025-07-05 / 10:50',
    },
];

function DeletedProductsPageWrapper() {
    ...
    const handleTableActions = (prop) => {
        ...
        if (action === 'restore') {
            console.log('Restore product:', data);
        } else if (action === 'view_details') {
            router.push(`/admin/products/${data.id}`);
        } else if (action === 'edit') {
            router.push(`/admin/products/edit/${data.id}`);
        } else if (action === 'delete_permanently') {
            console.log('Delete permanently:', data);
        }
    };
...
export default DeletedProductsPageWrapper;
